...
// === 新增：更新進度條與文字的函數 ===
    function updateProgress() {
        // 抓取畫面上所有的待辦事項、以及被標記為 completed 的事項
    ...
    sendBtn.addEventListener("click", function() {
        ...
        // 智慧判斷：如果輸入框以「待辦」開頭，或者有設定時間，就判定為待辦事項
        const isTodo = inputValue.startsWith("待辦") || timeValue !== "";

        if (inputValue === "") {
            aiResponse.textContent = "AI助理：請輸入指令或待辦事項";
            ...
            // 處理待辦事項邏輯
            let taskText = inputValue.replace(/^待辦\s*/, ""); // 過濾掉開頭的"待辦"字眼
            
            if (taskText === "") {
                aiResponse.textContent = "AI助理：請輸入待辦事項的具體內容！";
                return;
            }
            ...
            aiResponse.textContent = `AI助理：已新增待辦「${taskText}」！${timeValue ? '並設定了推播提醒。' : '單擊標記完成，雙擊刪除。'}`;
